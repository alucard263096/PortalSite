...
// 1. DEFINE VARIABLES AND PROPERTIES SPECIFIC TO THIS PAGE
	...
	if ($AccountUser) {
		if ($AccountUser->UserID == $Context->Session->UserID) {
			$Context->PageTitle = $Context->GetDefinition("Account");
		} else {
...
// 2. BUILD PAGE CONTROLS
	...
	if ($Context->Session->UserID > 0) {
		...
		if ($AccountUser && $Context->Session->User) {
			if ($AccountUser->UserID == $Context->Session->UserID) {
				$Panel->AddListItem($AccountOptions, $Context->GetDefinition("ChangePersonalInfo"), GetUrl($Configuration, $Context->SelfUrl, "", "", "", "", "PostBackAction=Identity"), "", "", 10);
		...
		if ( $AccountUser
			...
			$Panel->AddListItem($AccountOptions, $Context->GetDefinition("ForumFunctionality"), GetUrl($Context->Configuration, $Context->SelfUrl, "", "", "", "", "PostBackAction=Functionality"), "", "", 40);
		}
	}
