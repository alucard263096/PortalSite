...
/**
 * The DiscussionForm control is used for adding and editing discussions and comments in Guagua.
 * @package Guagua
 */
class DiscussionForm extends PostBackControl {
	...
	function DiscussionForm(&$Context) {
		...
		$this->Context->BodyAttributes .= ' onkeydown="if(event.ctrlKey && event.keyCode==13){document.getElementById(\'btnSave\').click();}"';
		...
		if ($this->DiscussionID == 0 && $this->Context->Session->UserID == 0) {
			...
			$this->FatalError = 1;
		} else if ($this->Context->Session->User->Permission('PERMISSION_HTML_ALLOWED')) {
			$Head->AddScript('editor/editor-aio.js');
			$Head->AddStyleSheet('editor/themes/default/editor-min.css', 'screen', 101, '');
		}
