...
class Email {
	...
	function Send($FatalError = 1) {
		...
		if ($this->ValidateEmail()) {
			...
			if ($this->Context->Configuration['SMTP_HOST'] != '') {
				...
				if (!@mail($To, $this->Subject, $Message, $Header) && $this->FatalError) $this->Context->ErrorManager->AddError($this->Context, $this->Name, "Send", "发送邮件时出错, 邮件发送未完成.", $php_errormsg);
			}
		}
	}
	...
	Rickard, I made small modifications for it to suit my needs for the Framework and it's coding standards.
	*/
	function ServerParse($Socket, $ExpectedResponse) {
		$ServerResponse = '';
		while (substr($ServerResponse, 3, 1) != ' ') {
			if (!($ServerResponse = fgets($Socket, 256))) {
				$this->Context->ErrorManager->AddError($this->Context, $this->Name, "ServerParse", "发送邮件时出错, 邮件发送未完成.", "邮件服务器无响应.");
			}
		}

		if (!(substr($ServerResponse, 0, 3) == $ExpectedResponse)) {
			$this->Context->ErrorManager->AddError($this->Context, $this->Name, "ServerParse", "发送邮件时出错. SMTP邮件服务器报告错误如下:", $ServerResponse);
		}
	}
	...
	function SMTPSend($To, $Subject, $Message, $Headers = '') {
		...
		if (!($Socket = fsockopen($SMTPHost, $SMTPPort, $ErrorNumber, $ErrorString, 15))) {
			if ($this->FatalError) {
				$this->Context->ErrorManager->AddError($this->Context, $this->Name, "SMTPSend", "无法连接SMTP邮件服务器 ".$SMTPHost.":".$SMTPPort, $ErrorNumber.": ".$ErrorString);
			} else {
				return false;
			}
		}
		...
		fwrite($Socket, 'MAIL FROM: <'.$this->Context->Configuration['SMTP_EMAIL'].'>'."\r\n");
		$this->ServerParse($Socket, '250');
