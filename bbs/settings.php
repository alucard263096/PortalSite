...
// 1. DEFINE VARIABLES AND PROPERTIES SPECIFIC TO THIS PAGE
	...
	if ($Context->PageTitle == '') $Context->PageTitle = $Context->GetDefinition('Settings');
...
// 2. BUILD PAGE CONTROLS
	...
	if ($Context->Configuration['USE_CATEGORIES']
		...
		) $Panel->AddListItem($AdminOptions, $Context->GetDefinition('Categories'), GetUrl($Configuration, 'settings.php', '', '', '', '', 'PostBackAction=Categories'), '', '', 50);
