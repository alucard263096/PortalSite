...
function RemoveComment($CommentID)
{
	...
	if($GLOBALS['DatabaseTables']['Attachment'] <>''){
		$AttachmentTbl = $GLOBALS['Configuration']['DATABASE_TABLE_PREFIX'].$GLOBALS['DatabaseTables']['Attachment'];
	}else{
		$AttachmentTbl = '';
	}
	...
	do
	{
		...
		if($CommentID == $ComInfo['first_com'])
		{
			...
			if($AttachmentTbl){
				$AttachmentData = $Context->Database->Execute(
					'select Path
					from '.$AttachmentTbl.'
					where DiscussionID = '.$ComInfo['id'].';', 
					'', '', 'An error occured while attempting to retrieve the comment status');

				while ($AttachmentInfo = $Context->Database->GetRow($AttachmentData)) {
					@unlink($AttachmentInfo['Path']);
				}

				if(!$Context->Database->Execute(
					'delete from '.$AttachmentTbl.' 
					where DiscussionID = '.$ComInfo['id'].';', 
					'', '', 'An error occured while attempting to remove Attachment')) break;
			}
		}
