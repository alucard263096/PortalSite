...
$Head->AddScript('js/guagua.js');
$Head->AddScript('js/ajax.js');
$Head->AddScript('js/ac.js');
$Head->AddStyleSheet($Context->StyleUrl.'guagua.css', 'screen', 100, '');

// BUILD THE MAIN MENU
$Menu->AddTab($Context->GetDefinition('MainPage'), 'discussions', GetUrl($Configuration, './'), '', $Configuration['TAB_POSITION_DISCUSSIONS']);
...
$CurrentCategoryID = ForceIncomingInt('CategoryID', 0);
if ($CurrentCategoryID == 0) {
	$DiscussionID = ForceIncomingInt('DiscussionID', 0);
	if ($DiscussionID != 0) {
		$GetCategoryIDbyDiscussionID = $Context->Database->Execute('select CategoryID from '.$Configuration['DATABASE_TABLE_PREFIX'] .$DatabaseTables['Discussion'].' where DiscussionID = '.$DiscussionID.' limit 1;', '', '', 'An error occured while attempting to retrieve the CategoryID.');
		$GetCategoryIDRows = $Context->Database->GetRow($GetCategoryIDbyDiscussionID);
		
		$CurrentCategoryID = $GetCategoryIDRows['CategoryID'];
	}else{
		$CurrentCategoryID = '';
	}
}
...
$Panel->AddString('<p id="AboutGuaGua">'.$Context->GetDefinition('PoweredBy').'</p>', 500);
