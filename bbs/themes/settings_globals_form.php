<?php
// Note: This file is included from the library/Guagua/Guagua.Control.GlobalsForm.php control.
echo '<div id="Form" class="Account GlobalsForm">';
	...
	echo '<fieldset>
		<legend>'.$this->Context->GetDefinition('ApplicationSettings').'</legend>
		'.$this->Get_Warnings().'
		...
		<h2>'.$this->Context->GetDefinition('FriendlyURL').'</h2>
		<ul>
			<li id="FriendlyURL">
				<p><span>'.GetDynamicCheckBox('URL_BUILDING_METHOD', 1, $this->ConfigurationManager->GetSetting('URL_BUILDING_METHOD'), '', $this->Context->GetDefinition('EnableFriendlyURL')).'</span></p>
				<p class="Description">'.$this->Context->GetDefinition('FriendlyURLNote').'</p>
			</li>
		</ul>
		...
		<ul>
			<li>
				...
				<input type="text" name="MAX_COMMENT_LENGTH" value="'.$this->ConfigurationManager->GetSetting('MAX_COMMENT_LENGTH').'" maxlength="255" class="SmallInput" id="txtMaxCommentLength" /> <span class="Description">'.$this->Context->GetDefinition('MaxCommentLengthNotes').'</span>
				<p class="Description">';
					$Selector->Clear();
					$Selector->CssClass = 'SmallSelect';
					for ($i = 1; $i < 31; $i++) {
					...
					echo str_replace(array('//1', '//2', '//3'),
						array($SecondsSelector->Get(), $Selector->Get(), $SecondsSelector2->Get()),
		...
		<ul>
			...
			<li>
				<label for="txtSMTPHost">'.$this->Context->GetDefinition('SMTPEmail').'</label>
				<input type="text" name="SMTP_EMAIL" value="'.$this->ConfigurationManager->GetSetting('SMTP_EMAIL').'" maxlength="255" class="SmallInput" id="txtSMTPEmail" />
			</li>
			<li>
		...
		<ul>
			<li>
				<label for="txtICPBeian">'.$this->Context->GetDefinition('WebICPBeian').'</label>
				<input type="text" name="WEB_ICPBEIAN" value="'.$this->ConfigurationManager->GetSetting('WEB_ICPBEIAN').'" maxlength="255" class="SmallInput" id="txtWebICPBeian" />
			</li>
			<li>
				<label for="txtWebPathToGuagua">'.$this->Context->GetDefinition('WebPathToGuagua').'</label>
				<input type="text" name="BASE_URL" value="'.$this->ConfigurationManager->GetSetting('BASE_URL').'" maxlength="255" class="SmallInput" id="txtWebPathToGuagua" />
