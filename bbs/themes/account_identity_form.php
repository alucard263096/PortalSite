<?php
// Note: This file is included from the library/Guagua/Guagua.Control.IdentityForm.php class.
...
if ($this->Context->Session->UserID != $this->User->UserID && !$this->Context->Session->User->Permission('PERMISSION_EDIT_USERS')) {
	...
	echo '<div id="Form" class="Account Identity">
		<fieldset>
			...
			<ul>';
				...
				if ($this->Context->Configuration['USE_REAL_NAMES'] == "1") {
					...
					<li>
						...
						<p class="Description"><span>'.GetDynamicCheckBox('ShowName', 1, $this->User->ShowName, '', $this->Context->GetDefinition('MakeRealNameVisible')).'</span>
						</p>
					</li>';
				}
				if ($this->Context->Configuration['ALLOW_EMAIL_CHANGE'] == '1') {
					echo '<li>
						...
						<input type="text" name="Email" value="'.$this->User->Email.'" maxlength="200" class="SmallInput" id="txtEmail" />
							...
							<p class="Description"><span>'.GetDynamicCheckBox('UtilizeEmail', 1, $this->User->UtilizeEmail, '', $this->Context->GetDefinition('CheckForVisibleEmail')).'</span>
						</p>
					</li>';
				} else {
					echo '<li>
			...
			echo '<p class="Description"><a href="javascript:AddLabelValuePair();">'.$this->Context->GetDefinition('AddLabelValuePair').'</a></p>
			<div class="Submit">
