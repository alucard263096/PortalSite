<?php
// Note: This file is included from the library/Guagua/Guagua.Control.CategoryForm.php control.

echo '<div id="Form" class="Account CategoryForm">
	<fieldset>
		<legend>'.$this->Context->GetDefinition('Categories').'</legend>'
		.$this->Get_Warnings()
	...
	if ($CategoryID > 0) {
		...
		echo '<h2>'.$this->Context->GetDefinition('GetCategoryToEdit').'</h2>
			<ul>
				<li>
					<label for="sCategorySelect">'.$this->Context->GetDefinition('CategoryManagement').' <small>'.$this->Context->GetDefinition('Required').'</small></label>
	...
	<li>
		<p class="Description">'."<a href=\"./\" onclick=\"CheckAll('RoleID_'); return false;\">".$this->Context->GetDefinition('All').'</a>&nbsp;&nbsp;&nbsp;'
