<?php
// Note: This file is included from the library/Guagua/Guagua.Control.CommentGrid.php class.
...
if ($this->Context->WarningCollector->Count() > 0) {
	...
	$CommentList .= '<div class="ContentInfo Top">
		<h1>';
			if ($this->Context->Configuration['USE_CATEGORIES']) $CommentList .= '<a href="'.GetUrl($this->Context->Configuration, 'index.php').'">'.$this->Context->GetDefinition('SiteMainPage').'</a> <span class="grey">&raquo;</span> <a href="'.GetUrl($this->Context->Configuration, 'index.php', '', 'CategoryID', $this->Discussion->CategoryID).'">'.$this->Discussion->Category.'</a> <span class="grey">&raquo;</span> ';
		...
		$CommentList .= '<li id="Comment_'.$Comment->CommentID.'"'.($CommentClass == ''?'':' class="'.$CommentClass.'"').'>
			...
			<div class="CommentHeader">
				...
				<span>
					...
					$CommentList .= '&nbsp;&nbsp;&nbsp;&nbsp;'.(($this->CurrentPage-1)*$this->Context->Configuration['COMMENTS_PER_PAGE']+$RowNumber).' <sup>#</sup>
				</span>
			</div>';
