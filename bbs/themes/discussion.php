<?php
// Note: This file is included from the library/Guagua/Guagua.Control.SearchForm.php
// class and also from the library/Guagua/Guagua.Control.DiscussionForm.php's
...
$DiscussionList .= '
<li id="Discussion_'.$Discussion->DiscussionID.'" class="Discussion'.($this->Context->Session->UserID <= 0?' NotLogin':'').$Discussion->Status.($Discussion->CountComments == 1?' NoReplies':'').($this->Context->Configuration['USE_CATEGORIES'] ? ' Category_'.$Discussion->CategoryID:'').($Alternate ? ' Alternate' : '').'">';
	$this->CallDelegate('PreDiscussionOptionsRender');
	$DiscussionList .= '<ul>
		<li class="DiscussionType">'.DiscussionPrefix($this->Context, $Discussion).'</li>
		<li class="DiscussionTopic"><a href="'.$UnreadUrl.'">'.$Discussion->Name.'</a></li>';
		...
		<li class="DiscussionComments">
			<span>'.$this->Context->GetDefinition('CommentCounts').' </span>'.$Discussion->CountComments.'
		</li>
